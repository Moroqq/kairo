...
import {
  getExpenseViews, addExpense, updateExpense, deleteExpense, togglePaid,
  monthlyTotal, remainingTotal,
  type ExpenseInput,
...
export function useExpenses() {
  return useQuery({
    queryKey: KEY,
    queryFn: () => ({ views: getExpenseViews(), total: monthlyTotal(), remaining: remainingTotal() }),
    staleTime: 10_000,
  });
}
...
export const useTogglePaid    = () => useExpenseMutation((id: string) => togglePaid(id));
